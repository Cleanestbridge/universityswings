...
document.addEventListener('click', (e)=>{
  ...
  const preBtn = e.target.closest?.('[data-prefill-request]');
  if (icsBtn){
    const title = icsBtn.getAttribute('data-title');
    const date  = icsBtn.getAttribute('data-date');
    const dt = new Date(date+'T14:00:00');
...
END:VCALENDAR`;
  ...
  if (preBtn){
    $('#rUniversity')?.value = preBtn.getAttribute('data-university') || '';
    $('#rCityState') ?.value = preBtn.getAttribute('data-citystate')  || '';
